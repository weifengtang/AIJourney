...
def is_running_in_claude_code() -> bool:
    ...
    # Claude Code 会设置特定的环境变量
    return os.environ.get('CLAUDE_CODE_ENV') == 'true' or \
           os.environ.get('ANTHROPIC_CLAUDE_CODE') == '1' or \
           os.environ.get('CLAUDE_PLUGIN_ENV') == 'true' or \
...
class AIJourneyConfig:
    ...
    def __init__(self):
        ...
        self._is_claude_code = is_running_in_claude_code()
    ...
    def _load_env_overrides(self):
        ...
        self._data_dir_override = os.environ.get('AIJOURNEY_DATA_DIR')
        self._log_dir_override = os.environ.get('AIJOURNEY_LOG_DIR')
        # LLM 配置
    ...
    def data_dir(self) -> Path:
        ...
        优先级：
        1. 环境变量 AIJOURNEY_DATA_DIR
        2. 配置文件 data_dir
        3. 项目根目录下的 data 文件夹
        """
        if self._data_dir_override:
            return Path(self._data_dir_override).expanduser().resolve()
        
        config_value = self._get_config_value('data_dir')
        if config_value:
            return Path(config_value).expanduser().resolve()
    ...
    def log_dir(self) -> Path:
        ...
        优先级：
        1. 环境变量 AIJOURNEY_LOG_DIR
        2. 配置文件 log_dir
        3. 项目根目录下的 logs 文件夹
        """
        if self._log_dir_override:
            return Path(self._log_dir_override).expanduser().resolve()
        
        config_value = self._get_config_value('log_dir')
        if config_value:
            return Path(config_value).expanduser().resolve()
    ...
    def llm_provider(self) -> str:
        ...
        优先级（优化后，优先使用 Claude Code 内置能力）：
        1. 环境变量 AIJOURNEY_LLM_PROVIDER（用户显式指定，优先级最高）
        2. 如果在 Claude Code 环境中，自动使用内置 LLM (claude_code_internal)
        3. 配置文件 llm.provider（仅当不在 Claude Code 环境时使用）
        4. 默认值: mock（演示模式，非 Claude Code 环境）
        """
        # 优先级 1: 环境变量（用户显式指定，优先级最高）
        if self._llm_provider_override:
            return self._llm_provider_override
        
        # 优先级 2: 在 Claude Code 环境中优先使用内置 LLM
        # 这是最重要的优化：在 Claude Code 中运行时，自动使用内置 AI 能力
        if self._is_claude_code:
            return 'claude_code_internal'
        
        # 优先级 3: 配置文件（仅当不在 Claude Code 环境时使用）
    ...
    def llm_enabled(self) -> bool:
        ...
        return bool(self.llm_api_key) or \
               self.llm_provider == 'mock' or \
               self.llm_provider == 'claude_code_internal'
