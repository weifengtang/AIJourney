...
class CodeBuddyCollector(BaseCollector):
    ...
    def collect(self, target_date: date, save_raw: bool = True) -> List[SessionData]:
        ...
        CodeBuddy 数据目录结构：
        - ~/Library/Application Support/CodeBuddy/storage (macOS)
        - %APPDATA%/CodeBuddy/storage (Windows)
        - 数据文件: {uuid}/{session_id}.json
        - 数据格式: JSON 格式，包含 messages 数组
        ...
        # 保存原始数据（保持完整，不剪辑）
        if save_raw and sessions:
            self.save_raw_data(sessions, data_path)
